...
const { DEFAULT_ASK_PHRASES, ASK_TAG, DONE_TAG, normalizePhrases, phraseSources, buildAskMatcher, callKind, saysDone, askExcerpt } = require('./ask-phrases');
...
// «Где я» — про ЧЕЛОВЕКА: за компом он или с телефоном. Ночного режима здесь больше нет, и это
// не уборка, а разделение двух разных вопросов. «Где я» решает, КУДА писать человеку; ночь
// решает, каким вкладкам можно работать БЕЗ него. Пока они делили одно поле, ответы спорили:
// человек с телефоном, а все вкладки отданы — что это за положение? Теперь ночь живёт на
// вкладках (d.auto), а «включена» она тогда, когда отданы все (см. nightAllOn).
const TG_PRESENCE = ['desk', 'phone'];
let tgPresence = 'desk';
// Идёт ли раздача мандата всем вкладкам разом. Флаг нужен, чтобы служебное («ночь включилась»,
// «ночь кончилась») случилось ОДИН раз на всю пачку, а не на каждой вкладке по дороге.
let nightSync = false;
// Ночь, доставшаяся от прежней версии (см. tgLoad). Живёт только до первой забранной вкладки.
let nightLegacy = false;
...
function tgLoad() {
  ...
  tgApplyPrompt();
  // Ни одно положение «где я» не переживает выключение приложения: «за телефоном», доставшееся
  // от вчерашнего вечера, значило бы, что агенты во всех вкладках отказываются показывать
  // варианты выбора человеку, который сидит перед маком. А ночь теперь и не положение — её
  // держат сами вкладки, и она возвращается вместе с ними.
  // Наследство: у прежних версий «меня нет» жило здесь, отдельным полем, и мандатов на
  // карточках при этом не было вовсе. Такие вкладки восстановятся «своими» — то есть ночь,
  // включённая вчера, молча кончилась бы при обновлении сворма. Поэтому флаг доживает ровно до
  // первой забранной вкладки: пока он в силе, восстановленная вкладка рождается отданной.
  if (TG.night) {
    nightLegacy = true;
    TG.night = false;
...
function tgApplyKeepAwake() {
  ...
  // Мандат, а не переключатель: держать мак бодрым надо ровно тогда, когда без человека
  // кто-то работает. Раньше здесь стояло общее положение, и вкладка, отданная одна, уезжала
  // в сон вместе с маком — то есть мандат ей выдали, а работать не дали.
  const want = autoAny()
...
function tgOnTabGone(d) {
  ...
  // Список вкладок изменился, а «ночь включена» считается по нему: закрытая «своя» вкладка
  // могла быть последней не отданной. Выходящее приложение не трогаем — там кончились не
  // вкладки, там кончилось приложение.
  if (!quitting) { nightPush(); tgApplyKeepAwake(); }
  nightClear(d);
  tgCancelWaiting(d);
...
function tgWriteModes() {
  ...
  // Вкладки с мандатом «работай без меня» — ВСЕ, и других списков у хука больше нет. Раньше
  // рядом жило «меня нет» отдельным положением, и хук смотрел то на список, то на него; теперь
  // ночь — это и есть мандаты вкладок, а «включена» она тогда, когда в списке все.
  const auto = [];
...
function tgOnWaiting(id) {
  ...
  // Исполнитель бригады спрашивает прораба (crewAskProrab), не человека — звать человека
  // только за разрешением: его прораб не раздаёт (спека, «Кто что видит»).
  if (d.parentId && d.waitingKind !== 'permission' && !d.tgAck) return;
...
async function tgNotifyDone(id, d, fresh) {
  // Итог исполнителя бригады — прорабу, не человеку: он его и так получает, а человек о каждом
  // «готов» каждого исполнителя только раздражался. Кроме случая, когда человек сам написал
  // исполнителю из чата (d.tgAck) и ждёт ответа именно там.
  if (d.parentId && !d.tgAck) return;
...
async function tgNightCmd(u) {
  ...
  const on = awayAll();
  const changed = setAllAuto(!on, 'телега') > 0;
  if (on) {
  ...
  await tgSend({ threadId: u.threadId, text: changed
    ? '🌙 Ночной режим у всех вкладок. Вопросы с вариантами агентам запрещу, прозу подтолкну'
      + ' правилом, дешёвые команды гита разрешу сам, лимиты разбужу по сбросу.'
      + ' Закончив задачу, каждая вкладка напишет итог — что сделала и что решила без вас.'
    : '🌙 Все вкладки и так отданы.' });
}
...
ipcMain.handle('telegram:forget', async () => {
  tgStop();
  // Бота забывают вместе со всеми настройками моста. Ночной режим в них больше не живёт: он на
  // вкладках, и отвязка группы его не трогает — гасить ночь отвязкой значило бы посреди неё
  // вернуть всем вкладкам обычные рамки.
...
// «Ночной режим включён» — это когда мандат есть у ВСЕХ вкладок.
// Отданы ли ВСЕ вкладки — то, что человек называет «ночной режим включён». Не хранится нигде:
// хранить его значило бы завести второе состояние на одно решение (см. night.nightOn).
function awayAll() {
  let tabs = 0;
  let autos = 0;
  for (const d of det.values()) {
  ...
  return night.nightOn({ tabs, autos });
}

// Есть ли мандат у ЭТОЙ вкладки. Ровно её собственная отметка — общего слоя над ней больше
// нет, и гадать, кто сейчас главный, не приходится.
function autoOn(d) { return !!(d && d.auto); }
...
function nightState() {
  return {
    // `on` — «без человека работают ВСЕ вкладки». Именно этим красится панель и этим
    // объясняется гейт ввода; про каждую вкладку в отдельности рендерер знает сам — он же их и
    // переключает. Отдельного состояния под этим полем нет, оно считается по вкладкам.
    on: awayAll(),
    typed: nightTyped,
...
function setTabAuto(id, on, from) {
  ...
  // (см. nightLegacy: оно доживает только до первого такого движения).
  if (!next) nightLegacy = false;
  d.auto = next;
  ...
  // «Ночь включилась / кончилась» — это переход СУММЫ: отданы все или уже не все. Служебное
  // (счётчики, напоминание, журнал) вешаем на переход, а не на каждую вкладку по дороге.
  if (!nightSync) nightFlip(wasAll, from);
  nightPush();
  tgApplyKeepAwake();
  return next;
}
...
function setAllAuto(on, from) {
  ...
  } finally { nightSync = false; }
  nightFlip(wasAll, from);
  nightPush();
  tgApplyKeepAwake();
  return touched;
}

// Служебное на переходе «ночь включилась / кончилась».
function nightFlip(wasAll, from) {
  const isAll = awayAll();
  if (isAll === wasAll) return;
  nightSwitch(wasAll, isAll, from);
}

// Ночь включилась или кончилась — то есть отданы стали все вкладки или уже не все. Здесь
// только СЛУЖЕБНОЕ: счётчики ночи, напоминание, журнал. Самих мандатов эта функция не трогает
// — их раздают две двери, полумесяц на карточке (setTabAuto) и «отдать все» (setAllAuto).
//
// Отдельного состояния «общий ночной режим» под ними больше нет. Пока оно было, на одно
// решение приходилось два ответа, и они спорили: карточка отмечена, общий снят — она ночная
// или нет? общий включён, карточка чистая — потеряю ли я её, когда вернусь?
function nightSwitch(wasAll, isAll, from) {
  if (isAll) {
    ...
    nightTyped = false;
    tgLog(`ночной режим (${from || '—'}): включён, вкладок ${autoCount()}`);
  } else if (wasAll) {
    ...
    tgLog(left
      ? `ночной режим (${from || '—'}): кончился, но ${left} вкладок остались отданными`
      : `ночной режим (${from || '—'}): кончился`);
  }
  nightPush();
}
...
ipcMain.handle('night:setAll', (_e, { auto } = {}) => setAllAuto(auto, 'окно'));
...
function tgSetPresence(raw, from) {
  ...
  // Положение больше не переживает выключение приложения — и ночь тоже не переживала БЫ его
  // здесь: она живёт на вкладках, а те сохраняются вместе с именем и папкой. Раньше в этом
  // поле хранилось «меня нет», и хранилось не от хорошей жизни: ночь, слетевшая в три часа от
  // автообновления, — это остаток ночи со стоящими вкладками. Теперь её держат сами вкладки.
  try { tgSave(); } catch (e) { reportMainError(e); }
...
function restartAsk(id, d, pct) {
  ...
  //
  // Полный askText — только на ПЕРВЫЙ заход этой серии (silent === 0 к этому такту). Переспрос
  // после молчания — это почти всегда агент, ответивший прозой в чат вместо файла: печатать ему
  // те же полсотни строк ещё раз ничего не чинит (см. askAgainText), а по три раза за серию
  // (MAX_SILENT) это и есть тот расход контекста, ради экономии которого весь перезапуск и
  // затевался.
