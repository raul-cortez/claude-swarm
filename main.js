...
function restartTick(id, d, now) {
  ...
  const r = restart.step(state, {
    ...
    status: d.status,
    dialog: !!parsePrompt(promptSnapshot(d)),
    shellBusy: d.shellBusy,
    modeVisible: !!readMode(snapshot(d)),
...
// Погасить прежнего агента. Диалог на экране проверил автомат — печатать при нём нельзя не из
// аккуратности: буквы `/exit` съест рамка запроса, а Enter, который идёт за ними через 90 мс,
// подтвердит подсвеченный в ней вариант. Приложение выдало бы разрешение, которого человек не
// видел, — самое дорогое, что здесь можно сделать.
function restartExit(id, d) {
  restartLog(`вкладка ${id}: печатаю /exit`);
  restartType(id, '/exit');
}
...
ipcMain.on('session:forgetLaunch', (_event, { id } = {}) => {
  const d = det.get(String(id == null ? '' : id));
  if (!d || !d.launchCmd) return;
  d.launchCmd = '';
  restartLog(`вкладка ${id}: агента запустили руками — забыл строку запуска`);
});
