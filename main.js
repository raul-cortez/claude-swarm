  ...
  for (const [id, d] of det) {
    ...
    try {
      ...
      if (next.status !== d.status || next.detail !== d.detail
        ...
        // И вторая, того же рода: досчитали сабагенты. Статус при этом не меняется вовсе — вкладка
        // всё это время «готова», — а держал перезапуск именно их счёт (см. turnOver в restart.js).
        if (isOver && (!wasOver || (sub === 0 && wasSub > 0))) restartKick(id, d, 'turn');
...
function tgOnWaiting(id) {
  ...
  if (d.rs && d.rs.phase === 'exiting') return;
...
function tgOnRestartExit(id, d) {
  ...
  tgCancelWaiting(d);
  if (!notified || TG.chatId == null || !TG.token) return;
  const threadId = tgTopicOf(d);
  tgLog(`  вкладка ${id}: перезапуск гасит уже объявленный вопрос — предупреждаю в чат`);
  tgSend({
    threadId,
    text: `♻️ ${tgTabName(id)} перезапускается — этот вопрос уехал вместе с сессией.`
      + '\nОтвечать на него уже некому: вкладка поднимется и спросит заново.',
    silent: true,
  }).catch(reportMainError);
}
...
function restartTick(id, d, now) {
  ...
  // папке вкладки — ещё и лишний повод для соседа увидеть чужой служебный файл.
  let answer = null;
  let file = '';
  ...
  const r = restart.step(state, {
    ...
    // Открыта ли РАМКА — по хуку, а не по пикселям. Клод объявляет событием оба своих запроса, и
    // ждать человека он умеет только в них: `perm` — разрешение, `ask` — коробка с вариантами.
    // Значит «последний сигнал хука — ожидание» и есть «рамка открыта», а больше её достоверно не
    // видит никто: зов прозой сюда не попадает вовсе (его приносят стенограмма и экран), и по
    // `kind` он от коробки неотличим.
    //
    // С выключенными хуками (это галочка в настройках) поле пустое, и рамка остаётся на одном
    // скрёбе — как было до этой правки. Закрыть дыру там нечем: единственный, кто отличает
    // коробку от зова, — сам Клод, а сказать об этом он может только событием.
    box: byPid ? false : !!(d.hookState && d.hookState.status === 'waiting'),
...
//
// Поводов ровно два, и разделены они не для журнала: защита от дребезга нужна одному и вредна
// другому.
...
//   'line' — рендерер принёс строку запуска. Событие ОДНОРАЗОВОЕ и ровно то, ради которого фаза
//            granted стоит, а приходит оно через миллисекунды после разрешения — то есть впритык
//            к пинку от статуса. Общая выдержка глотала его ровно в том случае, ради которого
//            пинок и заводился: вкладка стояла до общего такта, полминуты на глазах у человека.
