...
function restartTick(id, d, now) {
  ...
  const byPid = state.phase === 'exiting' && d.rsSignalled && d.rsKillPid;
  const r = restart.step(state, {
    ...
    shellBusy: byPid ? pidAlive(d.rsKillPid) : d.shellBusy,
    modeVisible: byPid ? false : !!readMode(snapshot(d)),
    // Уход просили сигналом, а не буквами: от этого зависит, сколько его ждать.
    signalled: !!d.rsSignalled,
...
// «процесс есть, но чужой» — для нас это тоже «жив».
...
//
// С сигналом ничего этого нет: поле ввода в схеме не участвует вовсе, а уход виден точно, по
// исчезновению pid. Не закрылся за отведённое время — не добиваем: вкладка остаётся живой и
// рабочей, а перезапуск отменяется до следующего круга (см. KILL_WAIT_MS в restart.js).
//
// Windows и всё, где `ps` недоступен: pid потомка узнать нечем, поэтому там остаётся прежний
// путь — напечатать `/exit` и ждать по экрану.
function restartExit(id, d) {
  ...
  // Спрашиваем потомков ЗАНОВО, а не берём их из общего обхода процессов. Тот обход идёт раз в
  // пять секунд, и его pid к этому моменту может принадлежать уже вышедшему процессу — а
  // система успевает выдать номер другому. Сигнал по такому номеру ушёл бы в чужой процесс, и
  // это единственная ошибка здесь, которую нельзя ни заметить, ни отменить.
  execFile('pgrep', ['-P', String(shellPid)], (err, out) => {
    const pid = Number(String(out || '').trim().split('\n')[0]);
    // pgrep нет (Windows) или потомков нет вовсе — остаёмся на прежнем пути.
    if (err || !pid) { typeInstead(err ? 'потомков оболочки не спросить' : 'в оболочке пусто'); return; }
    ...
    try {
      ...
      // Ушёл сам между вопросом и сигналом — не беда, автомат увидит пустую оболочку. Всё
      // остальное (нет прав) значит, что закрыть его нам нечем.
      typeInstead(`сигнал не прошёл (${e && e.code})`);
    }
  });
}
