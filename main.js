...
// Windows без `ps`: про процессы в оболочке мы там ничего не знаем (см. scanTabProcesses), и
// судить приходится по экрану. Выдержка нужна прежде всего потому, что `/exit` сам открывает меню
// команд Клода и оно закрывает строку режима — сразу после набора «мебели нет» всегда.
const RESTART_EXIT_BLIND_MS = 10_000;
// Первая проверка не раньше этого: Enter к /exit уходит через 90 мс после текста, и опрос без
// задержки заставал бы экран до нажатия.
const RESTART_EXIT_FIRST_MS = 1500;
// Сколько ждать заготовленный запуск, пока агент всё ещё жив. Ждём долго, потому что `/exit` из
// очереди никуда не денется. Но не вечно: человек мог утром вычистить строку ввода, и тогда
// выхода не будет вовсе, а через час-другой он сам закроет Клода — и получил бы наш запуск с
// ночной задачей. Отменяем только пока агент на месте: у пустой оболочки отмена оставила бы
// вкладку без агента.
const RESTART_PENDING_MS = 60 * 60 * 1000;
// Сколько раз подряд можно спрашивать вкладку, не получая ответа. Причина молчания бывает
// неустранимой: в режиме плана агент вообще не может писать файлы, а в ручном — упирается в
// разрешение на запись. Без счётчика мы будем вечно раз в двадцать минут вставлять в разговор
// двадцать строк просьбы, съедая ровно тот контекст, который взялись сберечь.
const RESTART_MAX_SILENT = 3;
// Насколько устаревший расход ещё годится для решения. Строгая свежесть (пять минут, как у
// /usage) здесь не работает: спрашиваем мы только отдохнувшую вкладку, а отдохнувшая вкладка
// статуслайн не перерисовывает — то есть после любой отсрочки число протухало, и функция
// замолкала для этой вкладки навсегда. Снимок привязан к разговору, так что после /clear или
// перезапуска он не подойдёт и число возьмётся заново.
...
// этот случай считаем (RESTART_MAX_SILENT) и говорим о нём в журнал, а не спрашиваем вечно.
//
...
function restartReadAnswer(id, d) {
  ...
  if (raw == null) {
    ...
    if (Date.now() - (d.rsAskedAt || 0) > restart.ANSWER_WAIT_MS) {
      d.rsSilent = (d.rsSilent || 0) + 1;
      if (d.rsSilent >= RESTART_MAX_SILENT) {
        // Молчит третий раз — причина не случайная. Скорее всего агент физически не может
        // записать файл: режим плана запрещает запись вовсе, ручной требует разрешения на
        // каждую. Спрашивать дальше значит переводить контекст на просьбы, которые некому
        // исполнить, — поэтому до конца этой сессии умолкаем сами.
        restartNote(id, `на просьбу о перезапуске нет ответа ${d.rsSilent} раза подряд — больше`
          + ' не спрашиваю. Похоже, агент не может записать файл: посмотрите режим разрешений');
        d.rsAskedAt = 0;
        d.rsRetryAt = 0;
        d.rsMute = true;
        return;
  ...
  // Разрешение — на ТОТ разговор, о котором спрашивали, и оно скоропортящееся. Считаем от
  // «сейчас», а не от времени вопроса: залежавшийся ответ написан ПОЗЖЕ вопроса, так что
  // сравнение с вопросом его как раз и пропускало. Случай живой: функцию выключили с висящим
  // вопросом, агент дописал ответ, файл остался лежать, через сутки функцию включили — и мы
  // стёрли бы разговор, в котором с тех пор работали целый день, по вчерашнему «можно».
  const age = Date.now() - (mtime || 0);
  if (!mtime || age > restart.ANSWER_WAIT_MS) {
    try { fs.unlinkSync(file); } catch (_) {}
    restartNote(id, 'ответ про перезапуск залежался — не перезапускаю, спрошу заново');
    restartLater(d);
    return;
  }
  const a = restart.parseAnswer(raw);
  // Удаляем только разобранное. Иначе тик, попавший в середину записи (эстафета текстом — это
  // килобайты, и запись не мгновенна), уничтожал бы уже дописанный агентом ответ: он об этом
  // не узнает, а следующий вопрос — через двадцать минут.
  if (!a) {
    // Одной строкой на попытку, а не на тик: ждём до срока годности выше, это двадцать заходов.
    if (d.rsUnparsed !== raw.length) {
      d.rsUnparsed = raw.length;
      restartLog(`вкладка ${id}: ответ пока не разобрать — жду (${raw.length} б)`);
    }
    return;
  }
...
setInterval(() => {
  ...
  for (const id of sessions.keys()) {
    ...
    // Недопечатанный запуск разбираем ДО проверки «включена ли функция». Заготовка появляется
    // только когда `/exit` уже ушёл агенту, и с этого момента вкладка обречена остаться голой
    ...
    if (d.rsPendingLine) {
      ...
      if (agentGone(d, d.rsExitAt)) { restartFire(id, d); continue; }
      // Агент всё ещё на месте час спустя — значит `/exit` до него не дошёл (например, человек
      // утром вычистил строку ввода). Ждать дальше нельзя: он закроет Клода сам через несколько
      // часов, и тогда в его оболочку приехал бы наш запуск с ночной задачей. Отменяем именно
      // здесь, пока агент ЖИВ, — у пустой оболочки отмена оставила бы вкладку без агента.
      if (now - (d.rsExitAt || 0) > RESTART_PENDING_MS) {
        restartNote(id, 'агент так и не вышел — запуск отменён, вкладка осталась как была');
        d.rsPendingLine = '';
        d.rsGoneSeen = 0;
        restartLater(d);
      }
    ...
    if (!RESTART_ENABLED || d.rsMute) continue;
...
function shellFree(id, d) {
  return new Promise((resolve) => {
    ...
    const wait = () => {
      ...
      if (agentGone(d, d.rsExitAt)) return resolve();
      if (Date.now() > until) return resolve();
      setTimeout(wait, 500);
    };
    // Первая проверка — не сразу. `/exit` в этот момент ещё даже не отправлен: restartType
    // печатает текст, а Enter к нему уходит через 90 мс. Опрос без задержки заставал экран до
    // нажатия — и на пути, где мы судим по экрану, это давало ответ «агент уже ушёл» мгновенно.
    setTimeout(wait, RESTART_EXIT_FIRST_MS);
  });
}
...
//
// На экранном пути осторожность двойная, потому что цена ошибки — строка запуска, уехавшая
// живому агенту репликой в разговор:
//   • не раньше слепой выдержки после /exit: сам `/exit` открывает меню команд Клода, и оно
//     закрывает собой строку режима — то есть «мебели нет» в первый же миг после набора;
//   • и не с первого взгляда: два подтверждения подряд, чтобы промежуточная перерисовка не
//     сошла за уход.
function agentGone(d, exitAt) {
  if (parsePrompt(promptSnapshot(d))) { d.rsGoneSeen = 0; return false; }
  if (d.shellBusy === false) return true;
  if (d.shellBusy !== undefined) { d.rsGoneSeen = 0; return false; }
  if (exitAt && Date.now() - exitAt < RESTART_EXIT_BLIND_MS) return false;
  if (readMode(snapshot(d))) { d.rsGoneSeen = 0; return false; }
  d.rsGoneSeen = (d.rsGoneSeen || 0) + 1;
  return d.rsGoneSeen >= 2;
}
...
ipcMain.handle('session:relaunch', async (_e, opts = {}) => {
  ...
  // Диалог на экране — и мы не печатаем НИЧЕГО. Это не про аккуратность: буквы `/exit` съест
  // рамка запроса, а Enter, который идёт за ними через 90 мс, подтвердит подсвеченный в ней
  // вариант. То есть приложение выдало бы разрешение, которого человек не видел, — самое дорогое,
  // что здесь можно сделать. Та же проверка стоит перед просьбой (restartAsk), перед командой из
  // телеги (tgTypeClaudeCommand) и перед сменой режима.
  if (parsePrompt(promptSnapshot(d))) {
    restartLog(`вкладка ${id}: на экране диалог — /exit не печатаю, перезапуск отложен`);
    restartLater(d, RESTART_BLOCKED_MS);
    return done({ ok: false });
  }
...
ipcMain.on('settings:restart', (_e, opts = {}) => {
  ...
  if (was && !RESTART_ENABLED) {
    ...
    // разговор, в котором с тех пор работали. Заготовленный запуск НЕ трогаем: там `/exit` уже
    // ушёл, и бросить вкладку голой оболочкой хуже, чем довести начатое.
    for (const [id, d] of det) {
      if (!d || !d.rsAskedAt) continue;
      d.rsAskedAt = 0;
      d.rsUnparsed = 0;
