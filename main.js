...
let tgPresence = 'desk';
// Где человек, когда он ЗДЕСЬ: за компом или за телефоном. Ночь сюда не пишется — она не
// место человека, а сумма вкладок (см. night.presenceFor), и возвращаться из неё надо туда,
// откуда ушли: сказавший «я за телефоном» и забравший вкладку остаётся за телефоном.
let tgManual = 'desk';
// Идёт ли перекладка мандатов из общего положения. Без этого флага общее положение и вкладки
// звали бы друг друга по кругу: положение раздаёт мандаты, каждый мандат пересчитывает
// положение.
let nightSync = false;
...
function tgOnTabGone(d) {
  ...
  // Список вкладок изменился — общее положение считается по нему (см. nightReconcile). Закрытая
  // «своя» вкладка может оказаться последней не отданной, и тогда человек ушёл; выходящее
  // приложение не трогаем — там вкладки не кончились, там кончилось приложение.
  if (!quitting) nightReconcile('вкладка закрылась');
  nightClear(d);
  tgCancelWaiting(d);
...
function setTabAuto(id, on, from) {
  ...
  // Общее положение — сумма вкладок, а не отдельный слой: забрали одну — человек вернулся,
  // отдали последнюю — его нет. Пересчитываем ЗДЕСЬ, в единственной двери к мандату вкладки.
  nightReconcile(from);
  nightPush();
  tgApplyKeepAwake();
  return next;
...
// Пересчёт общего положения по вкладкам. Ходит только от действий (мандат вкладки, рождение и
// смерть вкладки) — по таймеру этого делать нельзя: пустой список вкладок в первую секунду
// запуска погасил бы ночь до того, как вкладки восстановятся (см. night.presenceFor).
function nightReconcile(from) {
  if (nightSync) return;
  let tabs = 0;
  let autos = 0;
  for (const d of det.values()) {
  ...
  const want = night.presenceFor({ tabs, autos, manual: tgManual, presence: tgPresence });
  if (want && want !== tgPresence) tgSetPresence(want, from ? `${from} → счёт вкладок` : 'счёт вкладок', false);
}

// Общий ночной режим включили / выключили. Зовётся из tgSetPresence — единственной двери к
// положению «где я», — и разбирается только со СЛУЖЕБНЫМ: счётчики ночи, напоминание, журнал.
//
// Сами мандаты вкладок переписывает tgSetPresence, когда положение назвал человек, и не
// переписывает, когда положение пересчиталось по вкладкам (nightReconcile). Раньше здесь
// стояло обратное — общий режим лежал слоем ПОВЕРХ мандатов и их не трогал, — и выходили два
// состояния на одно решение: карточка отмечена, общий снят (она ночная или нет?), общий
// включён, карточка чистая (потеряю ли я её, когда вернусь?).
...
ipcMain.handle('tab:menu', (_e, { id } = {}) => {
  ...
  // починка: галочка вкладки теперь единственное состояние, а общее положение считается по
  // галочкам (см. nightReconcile). Снять её у последней отданной вкладки значит вернуться, и
  // никакого второго слоя, который «в силе», под ней не лежит.
...
function tgSetPresence(raw, from, syncTabs = true) {
  const next = TG_PRESENCE.includes(raw) ? raw : 'desk';
  // Человек назвал СВОЁ место — запоминаем его даже тогда, когда положение не поменялось:
  // из ночи возвращаются именно сюда.
  if (next !== night.NIGHT) tgManual = next;
  if (next === tgPresence) return false;
  const prev = tgPresence;
  tgPresence = next;
  // Общее положение — это и есть «отдать все вкладки разом» (или забрать их разом). Раньше оно
  // жило слоем поверх мандатов и их не трогало: карточки оставались чистыми, человек не мог
  // понять, что с ними будет, когда он вернётся. Теперь оно их переписывает — а обратно
  // положение считается по ним же (nightReconcile).
  //
  // syncTabs=false приходит из самого пересчёта: там вкладки уже такие, какие есть.
  if (syncTabs) {
    nightSync = true;
    try {
      for (const [id, d] of det) if (!d.dead) setTabAuto(id, next === night.NIGHT, from);
...
ipcMain.handle('session:create', (_event, opts = {}) => {
  ...
  d0.auto = !!opts.auto || awayAll();
  det.set(id, d0);
  if (d0.auto) tgWriteModes();
  nightReconcile('вкладка появилась');
