...
const selfRestartNote = (absPath) => {
  ...
  return [
    ...
    'Сначала — когда НЕ стоит, потому что это чаще путают: работа ЗАКОНЧЕНА, до конца осталось',
    'одно-два дешёвых действия, или СЕЙЧАС что-то прервётся вместе с тобой и потеряет токены',
    'впустую — работают сабагенты, идёт прогон команды, теста или сборки. Задача сделана — файла',
    'не пиши вовсе, просто закончи ход как обычно: свежая сессия на «продолжи» будет искать',
    'несуществующий остаток, а это чистый убыток на пустой старт.',
    'В остальных случаях решай сам: я вижу только проценты заполнения контекста, а объём работы',
    'впереди знаешь только ты. Если впереди большой кусок, а окно уже подъедено — начни его свежей',
    'сессией, а не тупеющей. Середина задачи (ревью, чек-лист, серия правок) — не причина ждать,',
    'рвать можно и посреди неё.',
    'Как: сначала запиши эстафету (что сделано, на чём стоишь, что дальше, чем проверять) туда,',
    `где у тебя это принято, потом ${where}`,
    'Файл — одним JSON-объектом:',
