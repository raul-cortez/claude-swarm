...
const selfRestartNote = (absPath) => {
  ...
  return [
    ...
    'НЕ стоит: работа уже ЗАКОНЧЕНА (файла не пиши вовсе, просто закончи ход — свежая сессия',
    'на «продолжи» будет искать несуществующий остаток, это чистый убыток на пустой старт);',
    'остались одно-два дешёвых действия; или СЕЙЧАС прервётся и сгорит впустую работа —',
    'сабагенты, прогон команды, теста, сборки.',
    'Иначе решай сам — я вижу только % контекста, объём работы впереди знаешь только ты.',
    'Большой кусок при подъеденном окне — начни его свежей сессией, а не тупеющей. Середина',
    'задачи (ревью, чек-лист, серия правок) — не причина ждать, рвать можно и посреди неё.',
    'Как: запиши эстафету (что сделано, на чём стоишь, что дальше, чем проверять) туда, где',
    `принято, затем ${where}`,
    'Файл — одним JSON, поле "model" в нём ОБЯЗАТЕЛЬНОЕ:',
    '{"restart": true, "model": "sonnet", "prompt": "чем займётся свежая сессия",'
      + ' "handoff": "где лежит эстафета"}.',
    'Подберу в течение полминуты, дождусь спокойного мига и подниму свежую сессию этим промптом.',
    'Без prompt или handoff (либо text с запиской) перезапуска не будет.',
    'Модель называй явно каждый раз — fable, opus, sonnet или haiku. Не назовёшь — останется',
    'прежняя, и вкладка, раз попав на дорогую, доживает на ней до конца задачи, сколько бы',
    'механики впереди ни было. Впереди механика (код, гейты, прогоны, ревью) — "sonnet";',
    'поднимаешься разговаривать со мной (развилка, приёмка, разбор) — "opus", это законно.',
  ].join('\n');
};
...
function outputFor(payload, matcher, tgSessions, presence, extra) {
  ...
  const restartIntro = (isStart && ex.restart && ex.restart.on)
    ? selfRestartNote(fileInfo && fileInfo.restart) : '';
