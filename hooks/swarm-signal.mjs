...
//
// Текст НЕ принимает резолвнутое имя файла и печатает один и тот же шаблон для всех сессий:
// SessionStart-инъекция харнесса кэшируется по точному совпадению префикса, и один байт
// разницы (например, sid внутри имени файла) рвёт кэш для ВСЕГО, что идёт следом, на каждой
// вкладке — см. PLAN-cache-friendly-hook-text.md. `$CLAUDE_CODE_SESSION_ID` агенту доступна
// как переменная окружения его же сессии, поэтому шаблон вместо резолвнутого имени ничего не
// теряет — `restartFileFor(sid)` при этом остаётся резолвером РЕАЛЬНОГО имени для main.js/restart.js.
const selfRestartNote = () => [
  ...
  'где у тебя это принято, потом подставь значение $CLAUDE_CODE_SESSION_ID (переменная окружения',
  'твоей же сессии, `Write` сам `$VAR` не разворачивает) и положи в рабочую папку файл',
  '.swarm-restart-$CLAUDE_CODE_SESSION_ID.json с одним JSON-объектом:',
...
function outputFor(payload, matcher, tgSessions, presence, extra) {
  ...
  const note = [starts ? usageNote(ex.usage, nowSec, subName(ex.subCards, ex.usage && ex.usage.home)) : '',
    ...
    needsDigestNudge ? digestMissingNudge() : '']
    .filter(Boolean).join('\n\n');
