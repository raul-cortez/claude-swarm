...
const nightRuleBody = () => [
  ...
  'Если вместо ответа человека тебе нужно просто ДОЖДАТЬСЯ чего-то в фоне (тест, сборка,',
  'подагент, которого сворм не видит на экране сам) — заканчивай ход отдельной строкой с',
  'тегом [swarm:фон]: иначе через пару минут тишины я решу, что ты встал, и приду спрашивать.',
].join(' ');
...
// куда положить файл.
//
// Платит за эти строки каждая сессия, поэтому их мало: здесь только то, по чему агент РЕШАЕТ,
// — когда стоит, когда нет и куда положить файл. Длинный протокол (что унести в эстафету, полная
// схема ответа, выбор модели) лежит в справке рядом со скриптом, `guide` — абсолютный путь до
// неё; читает её только тот агент, который уже решил перезапуститься, и читает один раз. Та же
// справка, тем же путём, приходит агенту из просьбы сворма на пороге (restart.js askText) — вход
// два, текст один. Нет справки (приложение старее скрипта) — говорим про эстафету одной строкой
// прямо здесь: без неё перезапуск состоится, но свежая сессия начнёт с пустого места.
//
// Текст по умолчанию печатает один и тот же шаблон для всех сессий: SessionStart-инъекция
// харнесса кэшируется по точному совпадению префикса, и один байт разницы (например, sid
// внутри имени файла) рвёт кэш для ВСЕГО, что идёт следом, на каждой вкладке — см.
// PLAN-cache-friendly-hook-text.md. `$CLAUDE_CODE_SESSION_ID` агенту доступна как переменная
// окружения его же сессии, поэтому шаблон вместо резолвнутого имени ничего не теряет —
// `restartFileFor(sid)` при этом остаётся резолвером РЕАЛЬНОГО имени для main.js/restart.js.
//
// `absPath`, если приложение его прислало (main.js: swarm-tgmode.json.files[sid].restart) —
// АБСОЛЮТНЫЙ путь до файла-ответа этой вкладки. Печатать его байт-в-байт безопасно для кэша:
// в отличие от sid, он не меняется при самоперезапуске (считается от ключа вкладки и её cwd —
// см. restartAnswerFile в main.js), значит внутри ОДНОЙ вкладки текст остаётся тем же самым
// между рестартами, а РАЗНЫЕ вкладки и так не делят этот кэш-сегмент (у них разные CLAUDE.md,
// память, codemap). Живой случай (fastio, 2026-08-31): агент работал в git worktree, «рабочая
// папка» из его точки зрения была не той, что отслеживает сворм (главное дерево), и файл ушёл
// не туда — самоперезапуск не сработал, а разбираться пришлось руками. Абсолютный путь эту
// догадку убирает целиком: агенту незачем знать, какая папка «его», — только куда писать.
const selfRestartNote = (absPath, guide) => {
  const where = absPath
    ? `положи АБСОЛЮТНЫЙ путь ${absPath} — пиши туда всегда, даже если сам сейчас в другой`
    ...
    : 'подставь значение $CLAUDE_CODE_SESSION_ID (переменная окружения твоей же сессии,'
      ...
      + ' .swarm-restart-$CLAUDE_CODE_SESSION_ID.json.';
  return [
    '[сворм] Ты можешь перезапустить себя сам, не дожидаясь, пока я спрошу: я вижу только %',
    'контекста, а объём работы впереди знаешь только ты. Большой кусок при подъеденном окне начни',
    'свежей сессией, а не тупеющей; середина задачи (ревью, чек-лист, серия правок) рвётся спокойно.',
    'НЕ стоит: работа уже ЗАКОНЧЕНА (файла не пиши вовсе, просто закончи ход — свежая сессия пойдёт',
    'искать несуществующий остаток); остались одно-два дешёвых действия; или СЕЙЧАС сгорит работа',
    '— сабагенты, прогон команды, теста, сборки.',
    guide
      ? `Решишь перезапуститься — прочитай ${guide}: что унести с собой и как ответить. Коротко —`
      : 'Решишь перезапуститься — запиши эстафету (что сделано, на чём стоишь, что дальше, чем'
        + ' проверять) туда, где принято. Затем коротко —',
    `${where}`,
    'Файл — одним JSON: {"restart": true, "model": "sonnet", "prompt": "чем займётся свежая',
    'сессия", "handoff": "где лежит эстафета"}. Модель называй явно каждый раз, иначе вкладка, раз',
    'попав на дорогую, доживает на ней до конца задачи.',
  ].join('\n');
};
...
//
// Шаблон, не резолвнутое имя — та же причина, что у selfRestartNote/digestNote: это печатается
// на КАЖДОМ ходе (не только на старте), пока файла нет, так что цена байта разницы даже выше.
// `absPath` — тот же абсолютный путь, что у selfRestartNote/digestNote, когда он известен: см.
// комментарий там про то, почему он безопасен для кэша и почему решает вопрос «какая папка моя».
const digestMissingNudge = (absPath) => absPath
  ? `[сворм] Дайджест вкладки (${absPath}) ещё не написан — заведи его прямо сейчас, не дожидайся паузы.`
  : '[сворм] Дайджест вкладки (.swarm-digest-$CLAUDE_CODE_SESSION_ID.json) ещё не написан — заведи его прямо сейчас, не дожидайся паузы.';
...
// `absPath`, если известен (см. selfRestartNote выше про тот же параметр) — проверяем ЕГО, а
// не payload.cwd + имя: агент в worktree-поддиректории даёт cwd, отличный от того, что
// отслеживает сворм, и старая проверка «дайджест не написан» тогда врала бы вечно — файл лежит
// по абсолютному пути, который сворм и правда читает, просто не там, где смотрел этот запасной
// путь.
const digestWritten = (payload, file, absPath) => {
  if (absPath) { try { return existsSync(absPath); } catch (_) { return true; } }
...
// тот же абсолютный путь и по той же причине, что у selfRestartNote (см. комментарий там).
const digestNote = (cfg, absPath) => {
  const max = digestMaxLenClamped(cfg && cfg.max);
  const own = String((cfg && cfg.note) || '').trim();
  const where = absPath
    ? `${absPath} — АБСОЛЮТНЫЙ путь, пиши туда всегда, даже если сам сейчас в другой папке`
      + ' (worktree, поддиректория задачи).'
    : 'файл .swarm-digest-$CLAUDE_CODE_SESSION_ID.json (переменная окружения твоей же сессии —'
      + ' сначала подставь её значение, `Write` сам `$VAR` не разворачивает).';
  return [
    ...
    `Обновляй, когда меняется этап работы (не на каждый шаг), перезаписывая в ${where}`,
    'Одним JSON-объектом: {"digest": "коротко и по-человечески, чем занят сейчас"}',
...
function outputFor(payload, matcher, tgSessions, presence, extra) {
  ...
  const sid = String((payload && payload.session_id) || '');
  // Абсолютные пути файлов ЭТОЙ вкладки, если main.js их прислал (swarm-tgmode.json.files[sid]
  // — карта пишется на каждое изменение claudeSessionId, см. tgWriteModes). Отсутствие — не
  // ошибка: старая версия main.js, гонка до первой записи карты — тогда ниже используется
  // прежний шаблон с $CLAUDE_CODE_SESSION_ID.
  const fileInfo = (ex.files && ex.files[sid]) || null;
  ...
  // Справка про перезапуск — рядом со скриптом, как и реестр соседей ниже: main.js кладёт в
  // userData и её, и сам скрипт, на каждом запуске. Нет файла — не указываем на него (тот же
  // принцип, что у peersIntro: обещать агенту дверь, которой нет, хуже, чем не обещать), и
  // selfRestartNote сам скажет про эстафету одной строкой вместо ссылки.
  const guideFile = fileURLToPath(new URL('./swarm-restart-guide.md', import.meta.url));
  const guide = existsSync(guideFile) ? guideFile : '';
  const restartIntro = (isStart && ex.restart && ex.restart.on)
    ? selfRestartNote(fileInfo && fileInfo.restart, guide) : '';
...
async function main() {
  ...
  try {
    ...
    fileMap = (tg.files && typeof tg.files === 'object') ? tg.files : null;
  } catch (_) { /* none */ }
