...
//
// Правило учит ТЕГАМ (ask-phrases.js ASK_TAG / WAIT_TAG / DONE_TAG). Метку в тексте
...
const { DEFAULT_ASK_PHRASES, TAG_NS, ASK_TAG, WAIT_TAG, DONE_TAG } = require('./ask-phrases');
...
function systemPromptRule() {
  return [
    ...
    `А если кончилась вся задача, а не её шаг, — начни итог строкой ${DONE_TAG}:`,
    'вкладка, которой отдали работать без человека, покажет этим, что работа сдана.',
...
module.exports = {
  DEFAULT_MARKER, TAG_NS, ASK_TAG, WAIT_TAG, DONE_TAG,
  systemPromptRule,
};
