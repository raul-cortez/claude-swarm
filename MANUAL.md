...
миграций, кому-то можно всё, кроме сети. Одну строку в правиле лучше оставить — ту, где агенту
велено начинать сообщение с `[swarm:вопрос]`. Это метка, по которой сворм отличает «агент ждёт
вас» от «агент закончил»: увидит её в начале строки — вкладка встанет «ждёт ответа», не увидит —
промолчит до утра. Настроить метку нельзя, это протокол.
