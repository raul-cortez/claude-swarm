...
| `/night` | ночь: агенты решают сами, разрешения стоят до утра (второй раз — снять) |
| `/morning` | сводка за ночь: кто стоит и что решили без меня |
...
Вечером вы загружаете вкладки работой на всю ночь. Проблема одна: любой агент
останавливается на первом же вопросе — и стоит до утра, даже если спрашивал про имя
переменной. Восемь вкладок, восемь потерянных ночей.

**Как включить.** В нижней панели, рядом с шестерёнкой, есть значок «где я». Нажмите и
выберите **ночь**. Вся нижняя панель станет синей — это видно от двери, и нужно это утром: вы
садитесь за стол, а приложение всё ещё считает, что вас нет. Бот в телеграме для ночи не
нужен, спать можно и без телефона. С телефона режим включает и снимает `/night`.

<!-- ФОТО · docs/img/20-night-bar.png · нижняя панель в ночном режиме: синяя, со значком луны. -->

**Что происходит ночью.** Агент, который собирается спросить вас с вариантами «1/2/3»,
получает отказ и вместе с ним правило: человека нет до утра; если решение обратимо или
переделка дешёвая — выбери сам и продолжай, а если ответ задаёт направление и ошибка стоит
дорого — сформулируй вопрос текстом и остановись. Отказ приходит **внутри хода**, поэтому
агент не встаёт вовсе: прочитал и пошёл дальше.

Если вопрос всё-таки задан текстом и вкладка встала, сворм один раз за ночь печатает в неё то
же правило. Один — и это важно: агент, услышавший правило в три часа, помнит его до утра. Встал
с вопросом уже после правила — значит вопрос настоящий, и вкладка спокойно стоит до утра, а
сворм с ней больше не спорит.

**Свои слова.** Оба ночных текста можно переписать — «Настройки → Ночь»: правило (то, что
агент получает вместо запрещённой рамки, и тем же текстом сворм толкает вставшую вкладку) и
вопрос замолчавшей вкладке. Пустое поле — заготовка сворма, она же стоит в поле серым, её удобно
скопировать и переписать под свой уклад: кому-то ночью нельзя касаться миграций, кому-то можно
всё, кроме сети. Одно место не свободно — метка вопроса: пишите в своём тексте
`{тег}` — сворм подставит ту метку, что настроена у вас.

**Чего ночь не делает.** Не отвечает за вас на запросы разрешения: там нужно выбрать вариант в
диалоге, а одобрять чужие команды молча — не то, за что стоит платить спокойным сном. Такая
вкладка стоит до утра, и в сводке видно, сколько она потеряла. И **не делает релизов** ни в
каком случае.
...
продолжаю сам (и говорит, что начал), нужно ваше слово. За ночь одна вкладка может так
продолжить не больше трёх раз, дальше ждёт вас.
...
честно — её ночь не трогает вовсе, сколько бы она ни стояла. И ответ вкладки за работу не
считается: сказала «всё сделано» — вопрос закрыт, второй раз про то же не спросят.

**Лимиты.** Упёрлась в лимит подписки — сворм знает время сброса от самого Клода и разбудит
вкладку сам, через пару минут после того, как окно откроется. Ночью это главная причина
простоя, и единственная, которую можно снять без вас. Если Клод дождался сброса сам и просит
нажать клавишу («press enter to continue»), сворм нажимает за вас — так же, не будя.

**Утро.** Снимите ночь тем же значком — и в панели появится «утро». Клик открывает сводку:
...
- внизу карточки, если было, — почему ночь эту вкладку **не трогала**.
...
<!-- ФОТО · docs/img/21-night-morning.png · окно «Утро»: карточки вкладок с половинами «на вас» и «без вас». -->

Вкладки, о которых сводке есть что сказать, помечены в полосе вкладок синей точкой — по ней видно, с
кем разбираться, даже когда окно закрыли. Кнопка «Прочитал» гасит и значок, и точки. С
телефона та же сводка приходит по `/morning` — сама она ночью не приезжает, чтобы не будить.

Если вы сели за клавиатуру, а ночь ещё включена, в панели появится «ночь включена»: агенты
продолжают решать за вас, пока вы сидите рядом. Клик снимает режим и показывает сводку.
