...
Служебное сворм допишет сам, и потерять его, переписав поле, нельзя. Это одна строка — про метку
`[swarm:вопрос]`, которой агент начинает сообщение, когда ждёт вас: по ней сворм и отличает «агент
ждёт» от «агент закончил». Строка показана под полями, целиком, но не редактируется — следить за
протоколом должен сворм, а не вы.
...
Такая вкладка стоит до вашего возвращения — сколько она простояла и на чём, видно на её экране.
И **релизов не делает** ни в каком случае.
