...
(function (root, factory) {
  ...
  // чтобы капсула не прыгала местами кнопок между перезапусками. «Закрыть» — всегда последней:
  // разрушительная кнопка стоит с краю, и её место не зависит от того, что включено перед ней.
  // «Переименовать» — полноправная кнопка, хотя имя переименовывается и двойным кликом: двойной
  // клик надо знать, а кнопку видно.
  const TOOL_KEYS = ['moon', 'prorab', 'resume', 'rename', 'close'];
  const TOOL_DEFS = [
    { key: 'moon', name: 'Работать без меня' },
    { key: 'prorab', name: 'Сделать прорабом' },
    { key: 'resume', name: 'Поднять упавший разговор' },
    { key: 'rename', name: 'Переименовать' },
