...
window.swarm.onStatus(({ id, status, detail, ctxPct, question, sub, waitingKind, sure, done }) => {
  ...
  s.rawStatus = status;
  s.rawDetail = detail;
  // Агент снова работает — значит вкладка поднялась (сама, руками или кнопкой подъёма), и
  ...
  // (готовая вкладка и так значит «свободна»), а вот у отданной вкладки это единственное
  // отличие «работает без меня» от «отработала»: см. paintAuto. Красим ПОСЛЕ applyStatus —
  // та тоже пишет в .sub, и, окажись paintAuto раньше, следующая строка стёрла бы её «работа
  // сдана» обратно на «готов».
  s.done = !!done;
...
function effectiveStatus(s) {
  ...
  // Агент упал, а оболочка осталась — и это сильнее любого наблюдения за экраном. Без этой
  // строки карточка через пару секунд закрашивалась обратно в спокойный «готов»: с точки зрения
  // скрёба пустая оболочка и есть свободная вкладка, и упавшая ночью работа выглядела бы точно
  // как вкладка, которой не давали задачи. Держим до тех пор, пока в ней снова не заведётся
  // агент (см. session:agentGone и снятие s.fallen ниже по файлу).
  if (s.fallen) return { status: 'dead', detail: s.fallen };
...
// Агент ушёл из ЖИВОЙ оболочки: `claude` кончился, а шелл остался с приглашением (main.js:
// agentGone). Вкладка при этом рабочая — в неё можно печатать, — но агента в ней нет.
window.swarm.onAgentGone(({ id, how, label, reset, canResume }) => {
  const s = sessions.get(String(id));
  if (!s || !s.term) return;
  // ⛔ Пишем в ЭМУЛЯТОР, а не в pty. Мусор вида `35;57;4M` в приглашении берётся оттого, что
  // xterm.js всё ещё считает мышиный режим включённым и шлёт события мыши в стандартный ввод
  // оболочки, а та честно пытается их исполнить. Гасить надо того, кто ШЛЁТ, — сам эмулятор;
  // та же строка, отправленная в pty, ушла бы zsh просто второй порцией мусора.
  ...
  s.done = false;
  applyStatus(s, { notify: false });
  paintAuto(s);
  // И в самой вкладке — словами. Карточка говорит, ЧТО случилось, но она мала; здесь место
  // сказать, что делать дальше. Тем же приёмом, что и «[session ended]» ниже по файлу.
  s.term.write('\r\n\x1b[2m[' + s.fallen + '. '
    + (canResume
      ? 'разговор цел — правый клик по карточке, «Поднять упавший разговор»'
      : 'поднять нечем: неизвестен id разговора или запускатель без --resume')
    + ']\x1b[0m\r\n');
});
...
async function createSession(opts = {}) {
  ...
  tab.innerHTML = `
    ...
    <span class="dot"></span>
    <!-- Луна вместо точки статуса на отданной вкладке. Отдельным элементом, а не кнопкой в
         капсуле: капсула прижата к правому краю, и по ховеру она РАСТЁТ ВЛЕВО (появляются
         разделитель и крестик) — луна уезжала из-под пальца, а на её место приходил крестик
         «закрыть вкладку». Здесь она отметка состояния, ничего не двигается, а кнопки в
...
function showSettingsModal(tab) {
  ...
  const save = () => {
    ...
    // applyStatus пишет в .sub статус агента, а на отданной сданной вкладке там должна
    // остаться «работа сдана» (см. paintAuto) — иначе настройка стирает эту подпись.
    sessions.forEach((s) => { if (s.alive) { updateAgents(s); applyStatus(s, { notify: false }); paintAuto(s); } });
...
// и из-за этого у ночного режима был слепой угол: четыре отданные вкладки выглядят одинаково и
// когда все четыре работают, и когда три уже сдали работу. Между ходами агент молчит, и
// молчание не отличить от конца задачи.
//
// Отличает их слово самого агента (тег конца задачи, см. DONE_TAGS в ask-phrases.js). Раньше
// карточка на это ещё и зеленела — тем же цветом, что готовая днём, — и путала: «зелёная,
// значит свободна» читалось раньше, чем «отдана, но сдала работу». Цвет теперь один и тот же,
// ночной, в обоих случаях; разница — только в подписи под именем (.sub), которую здесь и
// перехватываем поверх обычного статуса. Заработала снова — снова «работает без вас».
function paintAuto(s) {
  ...
  const sub = s.tab.querySelector('.sub');
  if (sub && done) sub.textContent = 'работа сдана';
  s.tab.title = !on ? ''
    : done
...
//
// В панели стоит короткий отсчёт («через 3д17ч») — мимоходом. Здесь время сброса словами
// (fmtResetWhen в subs.js): пятичасовое окно — всегда отсчётом («через 2 часа 15 минут»,
// часы на стене для него надо пересчитывать в уме), недельное — гибко: меньше суток до
// сброса — тот же отсчёт, иначе точные часы («завтра в 09:30» / «в четверг, 14:00») — далёкий
// сброс планировать удобнее по часам на стене, чем по числу «через N часов».
...
// «4 вкладок» — заметная небрежность на самом видном месте панели, и одного слова тут мало:
// в русском по числу согласуется вся фраза. Три помощника на три места, где это вылезало.
//
// Форма ПО ЧИСЛУ: 1 вкладка, 2–4 вкладки, 5+ вкладок (11–14 — тоже «вкладок»).
...
function tabsWord(n) { return plural(n, 'вкладка', 'вкладки', 'вкладок'); }
// Винительный падеж — для «забрать N вкладку»: с формой из tabsWord выходило «забрать
// 1 вкладка».
function tabsAcc(n) { return plural(n, 'вкладку', 'вкладки', 'вкладок'); }
// И глагол: «1 вкладка перейдут работать» — та же небрежность, только заметнее.
...
function renderNightPill(st) {
  ...
  // отдано, видно и по карточкам — они фиолетовые.
  nightPill.classList.toggle('is-quiet', !n);
  nightPill.classList.toggle('is-all', all);
  nightPill.classList.toggle('is-nag', all && !!nightNow.typed);
  nightPill.innerHTML = `<span class="ic">${ICONS.moon}</span>`;
  const say = (text, title) => { nightPill.title = title; };
  if (all && nightNow.typed) {
    ...
    say('', 'Вы за клавиатурой, а без вас работают ВСЕ вкладки. Клик — список: выключить'
      + ' ночной режим у всех.');
  } else if (all) {
    say('', 'Ночной режим включён у всех вкладок: решают обратимое сами, на дорогом'
      + ' останавливаются. Клик — список.');
  } else if (n) {
    ...
    say('', `Ночной режим у ${n} ${plural(n, 'вкладки', 'вкладок', 'вкладок')}: ${verb(n, 'решает', 'решают')}`
      + ` обратимое ${verb(n, 'сама', 'сами')}, на дорогом ${verb(n, 'останавливается', 'останавливаются')}.`
      + ' Клик — список.');
  } else {
    say('', 'Ночной режим: пусть вкладки работают без вас. Одну — полумесяцем на её карточке,'
      + ' все разом — отсюда.');
  }
  renderGate();
}
...
// брошенной работы, либо отданных вкладок, которых человек не отдавал.
//
// Пункты называют режим ЕГО ИМЕНЕМ — «включить ночной режим у всех вкладок», — а не действием
// вроде «отдать все». У глагола без адресата повисает вопрос: отдать кому, забрать у кого? А
// имя режима человек уже знает: оно стоит и на карточке, и в настройках, и в телеграме.
...
function nightMenuItems() {
  ...
  return [
    {
      id: 'all', icon: 'moon', name: 'включить ночной режим у всех вкладок',
      hint: total - n > 0
        ? `Ещё ${total - n} ${tabsWord(total - n)} ${verb(total - n, 'перейдёт', 'перейдут')} работать без вас.`
        : 'Уже отданы все.',
      off: total === 0 || n >= total,
    },
    {
      id: 'none', icon: 'monitor', name: 'выключить ночной режим у всех вкладок',
      hint: n
        ? `${n} ${tabsWord(n)} ${verb(n, 'вернётся', 'вернутся')} вам и ${verb(n, 'будет', 'будут')}`
          + ' ждать ответа на каждом шаге.'
        : 'Отдавать нечего.',
      off: !n,
    },
  ];
...
async function nightBulk(kind) {
  if (kind === 'none') {
    ...
    const ok = await confirmModal(
      `Забрать себе ${n} ${tabsAcc(n)}? ${verb(n, 'Она выйдет', 'Они выйдут')} из ночного режима`
        + ` и ${verb(n, 'перестанет', 'перестанут')} решать без вас — на каждом шаге`
        + ` ${verb(n, 'будет', 'будут')} ждать вашего ответа.`,
      'Забрать все',
    );
    if (!ok) return;
  }
