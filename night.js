...
function rule(tag) {
  ...
  return [
    ...
    `строкой с тегом ${m} — человек ответит, когда вернётся.`,
...
function phaseAsk(tag) {
  ...
  return [
    ...
    `(3) нужно решение человека — начни сообщение строкой ${m} и напиши вопрос, ответят позже.`,
...
// способа сказать «жду утра».
//
// {тег} — ПОДСТАНОВКА, а не то, чему учат. Она появилась, когда заготовка стояла в поле серой
// подсказкой: своё правило писали с чистого листа, и метку набирали руками — а разбор её строг
// ровно настолько, чтобы естественное «[сворм:вопрос]» не узналось, и вкладка молча простояла бы
// ночь с вопросом. Теперь заготовка лежит в поле значением, метка в ней уже написана буквально,
// и второе написание того же самого людей только путало. Подстановка осталась: чей-то
// сохранённый текст мог её застать, и печатать ему «{тег}» буквально нельзя.
