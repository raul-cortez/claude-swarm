...
// (ctx.auto); кто его выдал — она сама или общее «меня нет» — считает main.js.
//
...
// Третье положение «где я» (см. TG_PRESENCE в main.js): «меня нет». Значение осталось строкой
// 'night' — по нему живут сохранённые настройки и файл, который читает хук, и переименовывать
// его значило бы ломать чужие данные ради слова. Человек этого слова не видит: в интерфейсе
// положение называется «меня нет», а мандат вкладки — «авто».
const NIGHT = 'night';
...
function digest(entries, tabs, now, opts) {
  ...
  for (const t of live) {
    ...
    // вопросе»), хотя вопрос этот задан ЕМУ и он его видел. При общем «меня нет» мандат у всех, и
