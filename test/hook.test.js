...
// --- длинное про перезапуск живёт в справке, а не в контексте каждой сессии ------------------
// Строку про самоперезапуск платит СОБОЙ каждая вкладка, а пригождается она той одной, что
// реально решила рвать сессию. Значит здесь остаётся только то, по чему решают (когда стоит,
// когда нет, куда положить файл), а протокол эстафеты и полная схема ответа — в справке рядом
// со скриптом, куда сходит лишь тот, кто уже решил.
test('строка про самоперезапуск указывает на справку и сама протокол не пересказывает', () => {
  const guide = '/Users/x/Library/Application Support/swarm/swarm-restart-guide.md';
  const t = H.selfRestartNote('/repo/.swarm-restart-tab.json', guide);
  assert.ok(t.includes(guide), 'указатель на справку');
  assert.ok(t.length < 1200, `строка разрослась обратно: ${t.length} знаков`);
  assert.ok(/ЗАКОНЧЕНА/.test(t) && /сгорит работа/.test(t),
    'критерии «когда не стоит» остаются здесь: по ним решают ДО всякого чтения');
  assert.ok(/"restart": true/.test(t) && /"handoff"/.test(t),
    'и минимальный ответ тоже здесь — иначе справку придётся читать всем');
  assert.ok(!/отвергнутые варианты/.test(t) && !/договорились вернуться/.test(t),
    'длинный протокол уехал в справку');
});

// Приложение старее скрипта (или копия не удалась) — справки рядом нет. Указывать на файл,
// которого не прочесть, хуже, чем не указывать: агент потратит ход на Read и получит ошибку.
// Молчать про эстафету при этом нельзя — свежая сессия начнёт с пустого места.
test('без справки строка не ссылается в пустоту, но про эстафету говорит сама', () => {
  const t = H.selfRestartNote('/repo/.swarm-restart-tab.json', '');
  assert.ok(!/прочитай/.test(t), 'нет файла — нет и указателя на него');
  assert.ok(/эстафету/.test(t), 'зато сказано, что записку всё равно надо оставить');
});
