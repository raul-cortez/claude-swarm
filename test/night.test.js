...
// Без этой памяти вопрос повторялся бы каждые две минуты до утра: у ответа «всё сделано» свой
// ход, и проверка «про этот ход уже спрашивали» его не ловит.
test('ответившую «всё сделано» больше не спрашиваем', () => {
  const d = night.phaseDecision({ finished: true }, readyCtx());
  assert.strictEqual(d.act, 'skip');
  assert.match(d.why, /уже сказала/);
});
...
test('правило называет фразу-маркер пользователя, а не только умолчание', () => {
  const r = night.rule('Позови меня');
  assert.ok(r.includes('«Позови меня: …»'), r);
  assert.ok(night.rule('').includes('Сейчас от тебя'));
});
