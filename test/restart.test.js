...
// Зов человеку перезапуску больше не помеха (см. turnOver), значит заданный и не отвеченный
// вопрос умрёт вместе с разговором. Спасти его может только сам агент — и только если знает.
test('в просьбе сказано про незаданный вопрос к человеку', () => {
  const t = R.askText({ pct: 31, answerFile: '/tmp/a.json' });
  assert.ok(/спросил человека/.test(t) && /впиши вопрос в записку/.test(t));
});
