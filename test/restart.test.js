...
// На вкладке лежит ответ, которого человек ещё не видел (см. unread.js). Гасить её нельзя: он
// спрашивал, ответ ему нужен, а свежая сессия про этот разговор не знает — экран чистый, и
// человек даже не поймёт, что ему отвечали. Ждём без срока, это решение владельца.
test('непрочитанный ответ держит перезапуск', () => {
  assert.strictEqual(step(idle(), { unread: true }).action, 'nothing');
  assert.strictEqual(step(idle(), { unread: false }).action, 'ask');
});
...
// То же самое, но помеха — человек. Разрешение агент дал ночью, а утром человек написал во
// вкладку и получил ответ. Проверять непрочитанное только в момент вопроса мало: между вопросом
// и исполнением проходит до десяти минут, и разрешение с прошлой ночи снесло бы свежий ответ
// вместе со вкладкой — ровно тот случай, ради которого всё и делалось.
test('разрешение не исполняется поверх непрочитанного ответа', () => {
  const granted = { ...idle(), phase: 'granted', at: NOW - 1000, prompt: 'дальше' };
  const held = R.step(granted, sig({ hasLine: true, unread: true }));
  assert.strictEqual(held.action, 'nothing');
