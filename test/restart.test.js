...
test('в просьбе есть и путь ответа, и процент, и ответ на каждую из трёх веток', () => {
  const t = R.askText({ pct: 31, answerFile: '/tmp/swarm/answer.json', guide: '/tmp/g.md' });
  assert.ok(t.includes('/tmp/swarm/answer.json'));
  assert.ok(t.includes('31%'));
  // Полная схема полей уехала в справку, но ОТВЕТИТЬ агент должен уметь, ничего не читая:
  // две дешёвые ветки целиком здесь, у третьей — минимум, который автомат примет.
  assert.ok(t.includes('{"done": true}'), 'ветка «закончено» без чтения справки');
  assert.ok(/\{"restart": false, "retry": \d+\}/.test(t), 'ветка «не сейчас» без чтения справки');
  assert.ok(/"restart": true/.test(t) && /"prompt"/.test(t) && /"handoff"/.test(t),
    'ветка «можно» — минимальный ответ, который автомат примет');
  assert.ok(t.includes('/tmp/g.md'), 'и указатель на справку за остальным');
});

// --- длинное — в справке, а не в просьбе -----------------------------------------------------
// Развилка у просьбы трёхветочная, и две ветки из трёх отвечаются одной строкой JSON. Пока
// протокол эстафеты печатался вместе с ними, за него платили и те вкладки, что ответили «не
// сейчас», — по три раза за серию переспросов. Контракт: просьба коротка, справка полна.
test('просьба стала вдвое короче прежней полсотни строк', () => {
  const t = R.askText({ pct: 31, answerFile: '/tmp/a.json', guide: '/tmp/g.md' });
  assert.ok(t.length < 2200, `просьба разрослась обратно: ${t.length} знаков`);
});

test('без справки просьба не ссылается в пустоту, но ответить по ней всё равно можно', () => {
  const t = R.askText({ pct: 31, answerFile: '/tmp/a.json' });
  assert.ok(!/прочитай/.test(t), 'нет файла — нет и указателя на него');
  assert.ok(t.includes('{"done": true}') && /"restart": true/.test(t), 'а ответить всё равно есть чем');
});
...
// Всё нижеследующее проверяется теперь в СПРАВКЕ, а не в просьбе: содержание то же, читает его
// только та ветка, которой оно нужно. Файл едет с приложением (build.files: hooks/**/*) и
// копируется рядом с хуком на каждом запуске — если он исчезнет или усохнет, ветка «можно»
// останется без протокола молча, поэтому контракт закреплён тестом.
const GUIDE = fs.readFileSync(path.join(__dirname, '..', 'hooks', 'swarm-restart-guide.md'), 'utf8');

test('в справке велено сохранить наработки по процессу и дописывать', () => {
  assert.ok(/предусмотрено твоим процессом|знает процесс твоей команды/.test(GUIDE),
    'формат — дело процесса, не наше');
  assert.ok(/дописывай, а не переписывай/i.test(GUIDE));
  assert.ok(/отвергнутые варианты/.test(GUIDE) && /развилки/.test(GUIDE));
});

test('в справке сказано про оба хвоста разговора', () => {
  assert.ok(/спросил человека/.test(GUIDE) && /впиши вопрос в записку/i.test(GUIDE),
    'незаданный вопрос');
  assert.ok(/договорились вернуться/.test(GUIDE) && /обсудим/.test(GUIDE), 'отложенный разговор');
});

test('в справке описаны все поля ответа и выбор модели', () => {
  for (const key of ['restart', 'done', 'retry', 'handoff', 'text', 'prompt', 'model']) {
    assert.ok(GUIDE.includes(key), `в справке не описано поле ${key}`);
  }
  for (const m of ['fable', 'opus', 'sonnet', 'haiku']) {
    assert.ok(GUIDE.includes(m), `в справке не названа модель ${m}`);
  }
});
