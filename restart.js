...
(function (root, factory) {
  ...
  // Сколько ждём ухода агента, которого попросили сигналом. Тридцати секунд хватает любому
  // штатному закрытию с запасом; всё, что дольше, — это уже «не закроется», и ждать час, как
  // после напечатанного `/exit`, там незачем.
  ...
  function step(state, sig) {
    ...
    if (st.phase === 'exiting') {
      ...
      // Сколько ждать ухода — зависит от того, КАК мы его просили.
      //
      // Сигнал процессу (s.signalled) — разговор короткий: агент либо закрывается за секунды,
      // либо что-то держит его насмерть, и через полминуты ясно, что не закроется. Добивать не
      // будем: вкладка живая и рабочая, а перезапуск подождёт до следующего круга.
      //
      // Печать `/exit` — другое дело, там ждём час. Буквы могли не дойти вовсе (склеились с
      // недописанным текстом в поле ввода), и человек закроет агента сам через несколько часов —
      // вот тогда в его оболочку и приехал бы наш запуск с ночной задачей. Отменяем, пока агент
      // ЖИВ: у пустой оболочки отмена оставила бы вкладку без агента вовсе.
      if (now - (st.exitAt || 0) > (s.signalled ? KILL_WAIT_MS : PENDING_MS)) {
        return {
          state: { ...initial(), silent: st.silent, retryAt: now + RETRY_MS },
          action: 'drop',
          note: s.signalled
            ? 'агент не закрылся по сигналу — запуск отменён, вкладка осталась как была'
            : 'агент так и не вышел — запуск отменён, вкладка осталась как была',
        };
      }
    ...
    if (st.phase === 'granted') {
      ...
      if (s.status !== 'ready' || s.dialog || !agentPresent(s)) return { state: st, action: 'nothing' };
  ...
  return {
    ...
    MAX_SILENT, PENDING_MS, KILL_WAIT_MS, EXIT_BLIND_MS, GONE_GAP_MS, GRANT_WAIT_MS, GRANT_CALM_MS,
    RETRY_MIN_MS,
