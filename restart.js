...
(function (root, factory) {
  ...
  // Открыт ли на вкладке запрос, в который нельзя печатать. Два источника, и оба нужны:
  //   • dialog — рамка НАЙДЕНА на экране;
  //   • kind — вкладка ждёт РАЗРЕШЕНИЯ, и это говорит хук, а не пиксели.
  // Пока «ждёт» блокировало всё подряд, рамка была прикрыта дважды, и второй источник был не нужен.
  // Теперь зов сквозь эту проверку проходит — и запрос разрешения остался бы на одном скрёбе
  // экрана. Промахнись он (рамка уехала за край, чужая отрисовка), и наши двадцать строк с Enter
  // уехали бы В РАМКУ, то есть выбрали бы в ней ответ за человека.
  function boxOpen(s) {
    return !!s.dialog || s.kind === 'permission';
  }
