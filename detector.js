...
// Токены, которых здесь НЕТ, — не забытые, а безстатусные: маркер бывает нужен ради самих
// своих полей (id разговора, путь стенограммы), а не ради состояния вкладки. Такой `hello`
// шлёт SessionStart — он говорит «разговор теперь другой», и это всё, что о вкладке в этот
// миг известно (см. tokenFor в hooks/swarm-signal.mjs). Неизвестный токен applyHook пропускает
// молча, и вкладка сохраняет статус, который у неё был.
const HOOK_TOKEN = {
...
function arbitrate(d, now, snap) {
  ...
  // Ход кончился ВОПРОСОМ — так говорит файл, а хук сказал только «готов». Спорить тут, как и
  // с фоном ниже, не о чем: оба канала согласны, что ход кончился, и расходятся лишь в том,
  // почему. Хук при этом всегда выигрывал по свежести (его сигнал приходит по концу хода, то
  // есть позже последней записи), и вопрос из файла терялся — а дальше по функции «готов»
  // поднимал кто угодно: фраза про фон, живой подагент. Живьём это выглядело так: вкладка с
  // открытым вопросом и работающим подагентом внутри показывала «работает в фоне», причём
  // через такт — снова «ждёт ответа», потому что привязка к файлу отваливалась и возвращалась
  // (см. перепривязку в main). На карточке это мигало подписью «вопрос» ↔ «ждёт ответа».
  if (hs.status === 'ready' && tr && tr.status === 'waiting') {
    return hasPromptBox(snap) ? mkWaiting(snap, true) : fromTranscript(tr);
  }
