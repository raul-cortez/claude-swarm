...
<p align="center">
  <!-- Ссылки обновляет scripts/release.mjs, вместе с блоком DL в разделе «Установка».
       ...
       которой уже нет. -->
<!--DLTOP-->
  <a href="https://github.com/raul-cortez/claude-swarm/releases/download/v0.49.0/swarm-0.49.0-arm64.dmg"><b>⬇&nbsp;&nbsp;Скачать для macOS</b></a>
  &nbsp;·&nbsp;
  <a href="https://github.com/raul-cortez/claude-swarm/releases/download/v0.49.0/swarm-0.49.0-x64.exe"><b>⬇&nbsp;&nbsp;Скачать для Windows</b></a>
  <br>
  <sub>v0.49.0 · Apple Silicon · <a href="#установка">macOS: один шаг после скачивания</a></sub>
  <!--/DLTOP-->
</p>
...
| [**Работа без вас**](#когда-вас-нет-ночной-режим) | вкладке можно отдать задачу и уйти. Агент решает обратимое сам, на дорогой развилке останавливается, а вернувшись, вы читаете отчёт о том, что он решил. |
...
<!--DL-->
**Последняя версия: 0.49.0** · [все релизы](https://github.com/raul-cortez/claude-swarm/releases)

- **macOS** (Apple Silicon): [`swarm-0.49.0-arm64.dmg`](https://github.com/raul-cortez/claude-swarm/releases/download/v0.49.0/swarm-0.49.0-arm64.dmg)
- **Windows**: [`swarm-0.49.0-x64.exe`](https://github.com/raul-cortez/claude-swarm/releases/download/v0.49.0/swarm-0.49.0-x64.exe) — собирается в CI после тега
<!--/DL-->
...
Поэтому у вкладки есть **ночной режим**: работай без меня. Включается он двумя дверями, и они про разное.
...
**Всем сразу.** Значок «где я» в нижней панели открывает список из трёх положений; выберите **«ночной режим»**, и он включится у всех вкладок разом. Вся нижняя панель станет синей: это видно от двери, и нужно это прежде всего при возвращении, когда приложение всё ещё считает, что вас нет.
...
**Чего ночной режим не даёт.** Запросы разрешения Swarm за вас не одобряет: там нужно выбрать вариант в диалоге, а молча соглашаться на чужие команды — не то, за что стоит платить спокойным сном. Такая вкладка стоит до вашего возвращения, и в отчёте видно, сколько она потеряла и на чём.
...
Отчёт — про вкладки, которые вы **отдали**, а не про весь Swarm. Отдали одну — в отчёте будет она одна; включали ночной режим — в отчёте все. Вкладка, которую вы вели сами, в него не попадёт, даже если стоит на вопросе: этот вопрос задан вам, и вы его видели.
...
С телефона доступно то же самое, и командой одной и той же: `/night` в теме вкладки включает ночной режим ей, в общей теме — всем вкладкам сразу, вторым разом снимает. `/morning` присылает отчёт — сам он ночью не приезжает, чтобы не будить.
...
Это два первых положения того же значка «где я», у которого живёт и «ночной режим». От выбранного положения зависит, будет мост писать вам или молчать:
...
Значит включён общий ночной режим. В панели про это висит напоминание, клик по нему возвращает вас и показывает отчёт. Отдельные вкладки после этого продолжат сами — те, которым вы отдали задачу правым кликом.
