...
<p align="center">
  <!-- Ссылки обновляет scripts/release.mjs, вместе с блоком DL в разделе «Установка».
       ...
       которой уже нет. -->
<!--DLTOP-->
  <a href="https://github.com/raul-cortez/claude-swarm/releases/download/v0.42.0/swarm-0.42.0-arm64.dmg"><b>⬇&nbsp;&nbsp;Скачать для macOS</b></a>
  &nbsp;·&nbsp;
  <a href="https://github.com/raul-cortez/claude-swarm/releases/download/v0.42.0/swarm-0.42.0-x64.exe"><b>⬇&nbsp;&nbsp;Скачать для Windows</b></a>
  <br>
  <sub>v0.42.0 · Apple Silicon · <a href="#установка">macOS: один шаг после скачивания</a></sub>
  <!--/DLTOP-->
</p>
...
<p align="center">
  <sub>
    ...
    <a href="#когда-вас-нет-работает-без-меня">Работа без вас</a> &nbsp;·&nbsp;
...
| [**Работа без вас**](#когда-вас-нет-работает-без-меня) | вкладке можно отдать задачу и уйти. Агент решает обратимое сам, на дорогой развилке останавливается, а вернувшись, вы читаете отчёт о том, что он решил. |
...
<!--DL-->
**Последняя версия: 0.42.0** · [все релизы](https://github.com/raul-cortez/claude-swarm/releases)

- **macOS** (Apple Silicon): [`swarm-0.42.0-arm64.dmg`](https://github.com/raul-cortez/claude-swarm/releases/download/v0.42.0/swarm-0.42.0-arm64.dmg)
- **Windows**: [`swarm-0.42.0-x64.exe`](https://github.com/raul-cortez/claude-swarm/releases/download/v0.42.0/swarm-0.42.0-x64.exe) — собирается в CI после тега
<!--/DL-->
...
Полоска контекста отвечает на вопрос «кого пора сжимать». Полную картину показывает команда `/usage` в Telegram: все вкладки, самые полные сверху, плюс окна подписки на 5 часов и на неделю со временем сброса.
...
## Когда вас нет: «работает без меня»
...
Поэтому вкладке можно выдать **мандат**: работай без меня. У мандата две двери, и они про разное.

**Одной вкладке.** Правый клик по её карточке → «Работает без меня». Карточка получает синюю кромку слева и колечко вокруг точки статуса, так что видно, какие вкладки уже не ваши. Так и живёт обычный день: одной вкладке отдали задачу на вечер, в остальных сидите сами.

<p align="center"><img src="docs/img/22-tab-auto.png" width="250" alt="Вкладка с мандатом: синяя кромка и колечко у точки статуса"></p>

**Всем сразу.** Значок «где я» в нижней панели открывает список из трёх положений; выберите **«меня нет»**, и мандат поднимется у всех вкладок разом. Вся нижняя панель станет синей: это видно от двери, и нужно это прежде всего при возвращении, когда приложение всё ещё считает, что вас нет.

<p align="center"><img src="docs/img/20-night-bar.png" width="900" alt="Нижняя панель в положении «меня нет»"></p>

Общее положение не переписывает отметки вкладок: пока оно включено, сами работают все, а когда вы вернулись, каждая вкладка возвращается к своему выбору — те, которым вы отдали задачу, продолжают. Сколько их работает без вас прямо сейчас, показывает счётчик в панели, и клик по нему забирает все назад одним движением.
...
**Чего мандат не даёт.** Запросы разрешения Swarm за вас не одобряет: там нужно выбрать вариант в диалоге, а молча соглашаться на чужие команды — не то, за что стоит платить спокойным сном. Такая вкладка стоит до вашего возвращения, и в отчёте видно, сколько она потеряла и на чём.
...
Это не строгость ради строгости. В отданную вкладку печатает сам Swarm — правилом, вопросом про этап, будильником по лимиту, — а два писателя в одну строку ввода дают мусор вместо просьбы. Да и человек, который печатает в отданную вкладку, чаще всего просто забыл, что отдал её. Кнопка снимает отметку с этой вкладки, а если включено общее «меня нет» — и его; остальные вкладки продолжают сами.

### Что было без вас

Когда вкладок с мандатом не осталось — вы сняли «меня нет» или забрали последнюю себе, — в панели появляется **отчёт**.

<p align="center"><img src="docs/img/25-report-pill.png" width="300" alt="Значок отчёта в нижней панели"></p>

Клик открывает его: карточка на вкладку, и в каждой две половины — что вкладка оставила **на вас** и что решила **без вас**.
...
- **на вас** — вопрос целиком и сколько вкладка уже стоит, запрос разрешения, неотпустивший лимит, закрывшаяся вкладка;
- **без вас** — развилка дословно с вариантами, из которых агент выбирал, закрытый этап и то, что вкладка начала вместо него, лимит, с которого её разбудили, и чем она кончила ход. Это очередь на ревью и главная причина читать отчёт;
- внизу карточки, если было, — почему Swarm эту вкладку не трогал.

Порядок карточек — не по времени, а по тому, что нужно от вас: сперва те, что стоят и ждут, потом работавшие сами, последними тихие. Тихие даже не занимают карточку, они одной строкой в конце. Клик по карточке уводит прямо в неё, а вкладки, о которых отчёту есть что сказать, помечены в списке синей точкой в углу — по ней видно, с кем разбираться, даже когда окно закрыли.

Отчёт — про вкладки, которые вы **отдали**, а не про весь Swarm. Отдали одну — в отчёте будет она одна; включали «меня нет» — в отчёте все. Вкладка, которую вы вели сами, в него не попадёт, даже если стоит на вопросе: этот вопрос задан вам, и вы его видели.

**Свои слова.** Правило и вопрос замолчавшей вкладке можно переписать под свой уклад — «Настройки → Авто». Пустое поле означает заготовку Swarm, она же стоит в поле серым, так что её удобно скопировать и поправить: кому-то нельзя касаться миграций, кому-то можно всё, кроме сети.

<p align="center"><img src="docs/img/19-settings-auto.png" width="900" alt="Настройки → Авто"></p>

С телефона доступно то же самое: `/auto` в теме вкладки отдаёт её, `/night` включает и снимает «меня нет», `/morning` присылает отчёт. Сам отчёт ночью не приезжает, чтобы не будить.
...
| `/night` `/auto` | «меня нет» для всех вкладок и мандат для этой |
| `/morning` | отчёт: кто стоит и что решили без вас |
...
Это два первых положения того же значка «где я», у которого живёт и «меня нет». От выбранного положения зависит, будет мост писать вам или молчать:
...
<p align="center"><img src="docs/img/10-settings-launch.png" width="900" alt="Настройки → Запуск"></p>
...
Страница **«Авто»** отвечает за работу без вас: там переписываются правило и вопрос замолчавшей вкладке (см. [«Свои слова»](#когда-вас-нет-работает-без-меня)). Страница **«Телеграм»** — мастер моста из трёх шагов и кнопка, которой ставится распознавание голоса.
...
Так и задумано: вкладка, которой вы отдали задачу, вам не принадлежит, пока вы её не забрали. Нажмите «Забрать себе» в вопросе, который появился на первой клавише, — набранное уйдёт агенту целиком. Правый клик по карточке снимает мандат заранее.
...
Значит включено общее «меня нет». В панели про это висит напоминание, клик по нему возвращает вас и показывает отчёт. Отдельные вкладки после этого продолжат сами — те, которым вы отдали задачу правым кликом.
